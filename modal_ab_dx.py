"""Modal runner: clean A/B + per-shape including HEAD layer."""
import modal

app = modal.App("ab-dx-dispatch")
...
def run() -> str:
    ...
    r = subprocess.run([sys.executable, "tests/ab_dx_dispatch.py"],
...
def main():
    out = run.remote()
    open("/tmp/ab_dx.txt", "w").write(out)
    print(out)